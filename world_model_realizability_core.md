...
The design consequence is that a world-model-driven architecture needs a fallback *family* indexed by falsification class, not one brake: braking answers inertial surprises, retreat along the entry path answers quasi-static ones, and a certificate that cannot tell the classes apart will pick the wrong one. Whether that family admits a joint hypothesis-free terminal set is now the sharpest open question in this line, and it is a better question than the one this section set out to answer.
