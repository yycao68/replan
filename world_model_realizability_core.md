...
*Why this is architecturally new relative to P1.* P1's hierarchy is triggered by one signal — the margin falling below $m_{\mathrm{safe}}$. Here there are **two logically independent triggers**: margin exhaustion (a statement about the plan) and hypothesis falsification (a statement about the world model). A falsification event is not a margin event and must not be handled as one: the margin can be large and healthy at the exact moment the hypothesis it was computed under becomes false. Conflating them is the specific error this paper argues against, and Theorem 3 shows it has teeth.
...
## 5. What each theorem predicts, so the experiments are consequences rather than inventions

Stated here because the four parts are only judgeable if their empirical content is visible.

| Theorem | Falsifiable prediction | Measurement |
|---|---|---|
| **T1(iii)** | Tightening the world model's set width returns margin one-for-one; no residual conservatism is attributable to the certificate. | Sweep hypothesis width; regress recovered $\rho$ against set width; slope should be 1 with no offset beyond $\varepsilon_R$. |
| **T2** | Measured warning time matches $(\rho-\beta)/L$ within the rate-uncertainty ratio $\kappa$. | Inject progressive environment deviations at controlled rates $L$; measure $t_v - t_d$ against the predicted bracket. P1 already reports 0.32–0.42 s detection leads on two independent platforms, which the theory should retrodict. |
| **T3** | A fallback certified under $\mathcal{H}_E$ fails on falsification events that a fallback certified under $\mathfrak{E}_{\mathrm{wc}}$ survives. | Same scenario, two fallback constructions, measure safe-stop success rate under hypothesis violation. **This is the paper's decisive experiment** — it is the one whose outcome could refute the thesis. |

None of this needs a humanoid or hardware. It needs a simulated legged or wheeled platform, generated terrain, a world model emitting set-valued predictions, and scripted hypothesis violations. That is consistent with your two-stage plan: this paper is stage one and does not depend on Atlas.
...
| Triggers | one (margin) | two, logically independent (margin, falsification) |
...
| Platform | fixed-base manipulator | environment-rich mobile/legged simulation |
...
**My overall judgement.** T2 and T3 are, as far as I can tell without a literature search, genuine and non-obvious, and they are coupled: T2 says margin buys warning time and derives the threshold; T3 says the warning is only useful if the recovery does not share the failed assumption. Together they are a thesis, not a position — the four parts stand up. T1 is scaffolding and should be presented as such rather than inflated. The literature check in §7 is the gating task, and it should be done before any prose is written, because objection 1 could still redirect the whole framing.
