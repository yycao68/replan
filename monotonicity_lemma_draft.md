...
**Status:** The Lemma (§3) and its empirical grounding (§5) have now been folded directly into `predictive_realizability_paper_draft.md`'s §VI, immediately after Theorem 3's proof, using the sign-condition/dense-scan-fallback version (§4(a)). Notation matches the main draft's Theorem 3 (§VI). Raised in review, verified against the actual reduced-order implementation (`code/local_planner.py`) before being trusted — see the empirical section at the end.

**Update: §4(b)'s closed-form fallback has now been implemented in `code/local_planner.py`** (`_torque_decomposition_whole_route`, `_closed_form_lambda_candidates`), but in an EXTENDED form this file's own §4(b) sketch, and this file's §1 derivation, do not cover, and the result is more modest than this file's "recommended path for SCL/L-CSS" framing anticipated. Two findings from implementing and directly verifying it, not assumed:

1. **§1's decomposition (`tau_i(lambda) = A_i/lambda^2 + B_i`, "by the manipulator equation") does not match this benchmark's actual simulated dynamics.** `code/models/planar3r.xml` gives every joint real viscous damping (`dof_damping=0.3`), which the idealized manipulator equation omits entirely. Verified directly: with damping zeroed, the 2-term model reproduces `mj_inverse`'s real torque to numerical precision at every `lambda`; with damping present (the model actually used throughout `code/`), it is off by several percent, growing with `lambda` — enough to occasionally misplace a candidate relative to the true margin threshold. Fixed by extending to a third, linear-in-`x=1/lambda` term `D_i` (damping scales as `1/lambda`, not `1/lambda^2`, under retiming) — a genuine quadratic in `x`, fit from three `required_torque` evaluations per step rather than assumed from a specific passive-force formula, closing the residual to ~0.02 Nm (consistent with the smaller, non-quadratic Coulomb friction term the fit still can't capture exactly). §1's derivation should be read as applying only to a frictionless/dampingless idealization; a full rewrite adding the `D_i` term is a prerequisite before this file's math is fully accurate for `code/`'s actual dynamics, not yet done in the prose above.

2. **Even with the corrected, damping-aware model, the closed-form candidate set only sufficed in 21/1084 (1.9%) of the random scenarios where it was needed** — essentially unchanged from the naive (pre-fix) model's own 1.9%. The damping fix closed a real accuracy gap but did not address the actual bottleneck: the true supremum of the AGGREGATE `m_phys(lambda) = min` over many per-(joint,step) curves can occur at a pairwise crossing point between two different curves, not at either curve's own extremum — a structural lower-envelope limitation no per-curve closed form (however accurate) resolves, and with a 3-DOF arm and dozens of route steps this dominates in practice. Timing confirms this is not merely a hit-rate curiosity: the closed-form path's own candidate-evaluation cost is comparable to or exceeds the 41-point dense-grid fallback's cost, so on the paper's own timing-benchmark stress case the measured retime-only cost actually rose (~63ms → ~98ms) rather than falling. Full numbers, methodology, and the two concrete cases where a dense-grid safety net (kept, and confirmed load-bearing, not a leftover) rescued a candidate the closed-form set missed: `code/README.md`'s "Closed-form retiming candidates" section.

**Implication for item 6's "recommended path for SCL/L-CSS" framing (§4(b), below):** still worth pursuing for the spin-off paper's own more careful, likely simpler illustrative examples (fewer joints/steps means far less lower-envelope competition, and the exactness/no-idealization-assumption property is a genuine strength there), but the framing that it "turns Theorem 3's bisection into a provably correct two-phase procedure" replacing the dense scan needs qualifying: it is provably correct, but not shown to be cheaper, on a whole-route benchmark of this benchmark's scale. That qualification, not a retraction, is what the spin-off paper should carry forward.
