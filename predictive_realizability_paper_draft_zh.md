...
[8] R. Orsolino, M. Focchi, C. Mastalli, H. Dai, D. G. Caldwell, and C. Semini, "Application of Wrench based Feasibility Analysis to the Online Trajectory Optimization of Legged Robots," *IEEE Robotics and Automation Letters*, vol. 3, no. 4, 2018, doi:10.1109/LRA.2018.2836441. arXiv:1712.06833.

条目 [1]–[6] 均已针对其原始来源独立核实（GitHub 议题通过 `gh issue view` 直接拉取；论文通过 arXiv/出版商页面核查），是在本文的审阅过程中完成的，而非未经核实就直接照搬原始规划文档——具体核实内容和未能核实的部分，见*草稿状态与未决事项*第 1 项和第 4 项。[7] 与 [8] 是为填补 §II-D 中原先两处未给出引用的论断而新增的，通过文献检索定位，并对照 arXiv/出版商的元数据（标题、作者、期刊、年份、DOI）做了核对——但与 [1]、[4]、[5]、[6] 不同，二者均未被通读全文，其页码范围也未针对出版商原始记录独立核实；[7] 的页码范围尤其是转引自二次索引，而非出版商原始记录本身（见*草稿状态与未决事项*第 8 项）。
